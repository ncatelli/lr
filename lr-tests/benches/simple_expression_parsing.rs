...
fn generate_test_stream(len: usize) -> Vec<Terminal> {
    let fill_to = [[Terminal::Plus, Terminal::Zero]]
        .into_iter()
        .cycle()
        .take(len)
        .flatten();
...
fn fixed_size_expression(c: &mut Criterion) {
    let input = generate_test_stream(1);

    c.bench_function("fixed size input", |b| {
        b.iter(|| {
            let tokenizer = black_box((&input).iter().copied());
...
criterion_group!(benches, fixed_size_expression);
